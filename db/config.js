...
db.serialize(() => {
    ...
    db.run(`CREATE TABLE IF NOT EXISTS categories (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT,created_at DATETIME default CURRENT_TIMESTAMP,
        updated_at DATETIME default CURRENT_TIMESTAMP);`)
    db.run(`CREATE TABLE IF NOT EXISTS orders (id INTEGER PRIMARY KEY AUTOINCREMENT, customer_id INTEGER, menu_id INTEGER, qty INTEGER, order_date DATETIME default CURRENT_TIMESTAMP,
        created DATETIME default CURRENT_TIMESTAMP, updated DATETIME default CURRENT_TIMESTAMP, FOREIGN KEY (customer_id) REFERENCES customer(id),
